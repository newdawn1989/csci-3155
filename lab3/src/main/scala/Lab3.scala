  ...
  def eval(env: Env, e: Expr): Expr = {
    ...
    e match {
      ...
      case Call(e1,e2) => eToVal(e1) match {
        case Function(None, x, esh) => {
          eval(extend(env, x, eToVal(e2)), esh)
        }
        case Function(Some(x1), x2, esh) =>{
        	eval(extend(extend(env, x1, eval(env, e1)), x2, eval(env, e2)), esh)    
        	}
  ...
  def step(e: Expr): Expr = {
    ...
    //Unary
      ...
      case Binary(bop, v1, v2) if (isValue(v1) && isValue(v2)) => (bop, v1, v2) match {
        ...
        //DoInequalities
        case (bop @ (Lt | Le | Gt | Ge), v1, v2) => B(inequalityVal(bop, v1,v2))
        //DoEquality
        ...
        //DoAndOr
        case (bop @(And|Or), v1,v2) => (bop, v1,v2) match {
          case(And, v1,v2) => if (toBoolean(v1)) v2 else v1
          case(Or, v1,v2) => if (toBoolean(v1)) v1 else v2
        }
      }
      //DoCall/CallRec
