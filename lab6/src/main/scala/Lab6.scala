...
object Lab6 extends jsy.util.JsyApplication {
  ...
  object RegExprParser extends Parsers {
    ...
    def intersect(next: Input): ParseResult[RegExpr] = concat(next) match {
      case Success(r, next) => {
        def intersect(acc: RegExpr, next: Input): ParseResult[RegExpr] =
          if (next.atEnd) Success(acc, next)
          else (next.first, next.rest) match {
            case ('&', next) => concat(next) match {
              case Success(r, next) => intersect(RIntersect(acc, r), next)
    ...
    def concat(next: Input): ParseResult[RegExpr] = not(next) match {
      case Success(r, next) => {
        def concat(acc: RegExpr, next:Input): ParseResult[RegExpr] = 
          if (next.atEnd) Success(acc, next)
          else not(next) match {
            case Success(r, next) => concat(RConcat(acc, r), next)
            case _ => Success(acc, next)
          }
        concat(r, next)
      }
    ...
    def star(next: Input): ParseResult[RegExpr] = atom(next) match {
      case Success(r, next) => {
        def star(acc: RegExpr, next: Input): ParseResult[RegExpr] =
          if (next.atEnd) Success(acc, next)
          else (next.first, next.rest) match {
            case('+', next) =>star(RPlus(acc), next)
            case('?', next) => star(ROption(acc), next)
            case('*', next) => star(RStar(acc), next)
             case _ => Success(acc, next)
          }
        star(r, next)
      }
  ...
  def retest(re: RegExpr, s: String): Boolean = {
    def test(re: RegExpr, chars: List[Char], sc: List[Char] => Boolean): Boolean = (re, chars) match {
      ...
      /*nontermination thing remchars needs to get smaller if remchars not smaller than chars false*/
      case (RStar(re1), _) => sc(chars) || test(re1,chars, {remchars => if(remchars.size<chars.size) test(RStar(re1), remchars, sc) else false} )
